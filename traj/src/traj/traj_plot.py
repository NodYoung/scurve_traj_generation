...
def plot_segment(figure, position, velocity, acceleration, jerk, n_points=500, j_max=None,
    ...
    for joint_i in range(positions.shape[1]):
        ...
        axes[0].plot(plot_times, positions[:,joint_i], c=c, linewidth=0.5, marker='o', markersize=1.0)
        axes[0].set_ylabel('position')
        axes[1].plot(plot_times, velocities[:,joint_i], c=c, linewidth=0.5, marker='o', markersize=1.0)
        axes[1].set_ylabel('velocity')
        axes[2].plot(plot_times, accelerations[:,joint_i], c=c, linewidth=0.5, marker='o', markersize=1.0)
        axes[2].set_ylabel('acceleration')
        axes[3].plot(plot_times, jerks[:,joint_i], c=c, linewidth=0.5, marker='o', markersize=1.0)
        axes[3].set_ylabel('jerk')

    axes[0].vlines(boundaries, positions.min(), positions.max(), color=(0.8, 0.8, 0.8))

    if p_max is not None:
        axes[0].plot(plot_times, [p_max] * len(plot_times), '--', color='k')
        axes[0].plot(plot_times, [-p_max] * len(plot_times), '--', color='k')

    if v_max is not None:
        axes[1].plot(plot_times, [v_max] * len(plot_times), '--', color='k')
        axes[1].plot(plot_times, [-v_max] * len(plot_times), '--', color='k')
        axes[1].vlines(boundaries, -v_max, v_max, color=(0.8, 0.8, 0.8))

    if a_max is not None:
        axes[2].plot(plot_times, [a_max] * len(plot_times), '--', color='k')
        axes[2].plot(plot_times, [-a_max] * len(plot_times), '--', color='k')
        axes[2].vlines(boundaries, -a_max, a_max, color=(0.8, 0.8, 0.8))

    if j_max is not None:
        axes[3].plot(plot_times, [j_max] * len(plot_times), '--', color='k')
        axes[3].plot(plot_times, [-j_max] * len(plot_times), '--', color='k')
